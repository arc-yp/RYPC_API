...
const STORAGE_KEY = 'pricing_plans_local';
...
class PricingService {
  ...
  async getPlanByType(type: 'first_year' | 'renewal'): Promise<PricingPlan | null> {
  ...
  async updatePlan(plan: PricingPlan): Promise<boolean> {
    ...
    try {
      ...
      if (index !== -1) {
        ...
        console.log('Pricing plan updated in localStorage');
        return true;
      }
      
      return false;
    } catch (error) {
      console.error('Error updating pricing in localStorage:', error);
      return false;
    }
  }
  ...
  async initializeDefaults(): Promise<boolean> {
    ...
    if (plans.length === 0) {
      ...
      // Fallback to localStorage
      localStorage.setItem(STORAGE_KEY, JSON.stringify(DEFAULT_PRICING));
      return true;
    }
