...
export const storage = {
    ...
    try {
      ...
      if (isSupabaseConfigured() && supabase) {
        try {
          ...
          console.error(
            "Supabase connection failed, using localStorage:",
            supabaseError
          );
          const tombstones = this._getDeletedIds();
          return this._getLocalCards().filter(
            (c) => !tombstones.includes(c.id)
          );
        }
      } else {
    ...
    try {
      ...
      if (isSupabaseConfigured() && supabase) {
        try {
          ...
          if (error) {
            console.error(
              "Error adding card to Supabase (keeping in localStorage):",
              error
            );
          ...
          console.error(
            "Supabase connection failed (keeping in localStorage):",
            supabaseError
          );
          // Card is already in localStorage, so return true
          return true;
    ...
    try {
      ...
      if (isSupabaseConfigured() && supabase) {
        try {
          ...
          if (error) {
            console.error(
              "Error updating card in Supabase (keeping localStorage changes):",
              error
            );
          ...
          console.error(
            "Supabase connection failed (keeping localStorage changes):",
            supabaseError
          );
          // Card is already updated in localStorage
          return true;
        ...
        console.log(
          "Supabase not configured, card updated in localStorage only"
        );
        return true;
      }
    } catch (error) {
    ...
    try {
      ...
      if (isSupabaseConfigured() && supabase) {
        try {
          ...
          if (error) {
            console.error(
              "Error deleting card from Supabase (keeping localStorage changes):",
              error
            );
        ...
        console.log(
          "Supabase not configured, card deleted from localStorage only"
        );
        return true;
      }
    } catch (error) {
  ...
  async migrateFromLocalStorage(): Promise<void> {
    ...
    try {
      ...
      if (testError) {
        console.error(
          "Supabase connection test failed, skipping migration:",
          testError
        );
        return;
      }
      ...
      console.log(
        `Starting migration of ${localCards.length} cards from localStorage to Supabase...`
      );
      ...
      for (const card of localCards) {
        try {
          ...
          if (checkError) {
            console.error(
              `Failed to check existing card: ${card.businessName}`,
              checkError
            );
            failCount++;
            continue;
          ...
          if (existingCard) {
            console.log(
              `Card already exists in Supabase: ${card.businessName}`
            );
            successCount++;
            continue;
          }
          ...
          if (error) {
            console.error(
              `Failed to migrate card: ${card.businessName}`,
              error
            );
            failCount++;
          } else {
          ...
          console.error(
            `Error migrating card: ${card.businessName}`,
            cardError
          );
          failCount++;
        }
      ...
      console.log(
        `Migration completed: ${successCount} successful, ${failCount} failed`
      );
      ...
      if (failCount === 0) {
        ...
        console.log(
          "Some cards failed to migrate - keeping localStorage as backup"
        );
      }
    } catch (error) {
  ...
  async syncData(): Promise<void> {
    try {
      ...
      const now = Date.now();
      const expiredActiveCards = transformedSupabaseCards.filter(
        (c) =>
          c.active !== false && c.expiresAt && Date.parse(c.expiresAt) < now
      );
      if (expiredActiveCards.length > 0) {
      ...
      console.log(
        `Data sync completed successfully - ${transformedSupabaseCards.length} cards synced`
      );
    } catch (error) {
  ...
  async incrementViewCount(cardId: string): Promise<boolean> {
    try {
      ...
      if (cardIndex !== -1) {
        localCards[cardIndex].viewCount =
          (localCards[cardIndex].viewCount || 0) + 1;
      ...
      if (isSupabaseConfigured() && supabase) {
        try {
          ...
          if (error) {
            console.error(
              "Error incrementing view count in Supabase (keeping localStorage changes):",
              error
            );
            ...
            if (!fetchError && currentCard) {
              ...
              if (updateError) {
                console.error(
                  "Error updating view count manually:",
                  updateError
                );
              } else {
        ...
        console.log(
          "Supabase not configured, view count updated in localStorage only"
        );
        return true;
      }
    } catch (error) {
    ...
    try {
      ...
      if (isSupabaseConfigured() && supabase) {
        try {
          ...
          console.error(
            "Error fetching view count from Supabase:",
            supabaseError
          );
        }
      }
